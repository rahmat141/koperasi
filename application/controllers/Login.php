...
class Login extends CI_Controller
{
            ...
            if ($user) {
                ...
                if ($user['status_anggota'] == 1) {
                    ...
                    $this->session->set_flashdata(
                        'pesan',
                        'Silah cek email anda untuk reset password'
                    );
    ...
    private function _sendEmail($token, $type)
    {
        ...
        $this->email->from('koperasiQu@gmail.com', 'Admin KoperasiQu');
        ...
        if ($type == 'verify') {
            ...
            $this->email->message('Klik link ini untuk mengubah password akun anda : <a href="' . base_url() . 'Login/resetPassword?email=' . $this->input->post('email') . '&token=' . urlencode($token) . '">Reset Password</a> <br> Token aktivasi akan berlaku selama 2x24 jam, lebih dari itu token tidak akan berlaku kembali');
        }
    ...
    public function changePassword()
    {
        ...
        $this->form_validation->set_rules('password', 'Paassword', 'trim|required|min_length[3]', [
            // 'matches' => '',
            'min_length' => 'Password harus lebih dari 3 karakter',
            'requried' => 'Password tidak boleh kosong',
        ]);
        $this->form_validation->set_rules('password2', 'Confirm Paassword', 'trim|required|min_length[3]|matches[password]', [
